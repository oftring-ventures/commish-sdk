...
import { verifySdkReads, readsConsumerScope } from "./verify-sdk-reads.mjs";
...
function installed(files, use) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "commish-reads-test-")));
  try {
...
test("Read scope requires the validated runtime root and all promised targets", () => {
  for (const root of [null, "types", "index"])
    for (const webhook of [false, true]) {
      const files = packed(root, webhook);
      if (root !== "index")
        assert.deepEqual(
          verifySdkReads("unused", files, () => assert.fail()),
          [],
        );
      else
        installed(files, (consumer, sdk) => {
          let calls = 0;
          assert.deepEqual(
            verifySdkReads(consumer, files, (command, args, options) => {
              calls++;
              assert.equal(command, process.execPath);
              assert.deepEqual(args, [
                join(consumer, "reads-probe.mjs"),
      ...
      for (const name of [...files.keys()].filter((name) => name !== "package/package.json")) {
        ...
        missing.delete(name);
        assert.throws(
          () => verifySdkReads("unused", missing, () => assert.fail()),
          /missing promised SDK target/,
        );
      }
    }
});
test("Read execution rejects altered bytes, escaped resolution, invalid shapes and failures", () => {
...
test("actual Node import cannot accept a runtime root missing its read interface", () => {
  const files = packed("index", false, "export class Commish {}");
  installed(files, (consumer) => {
    assert.throws(
      () => verifySdkReads(consumer, files),
      (error) => error.status === 1 && error.stderr.toString().includes("public read interface"),
    );
  });
});
